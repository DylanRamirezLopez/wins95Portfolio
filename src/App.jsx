...
import badword from './data/badword'
...
import iconInfo from './data/icon.json'
...
function App() {
  ...
  const deleteIcon = ['Cat', 'AiAgent','Winamp','Paint','3dObject','Patch','Nft'];
  ...
  try {
    ...
    const iconNames = new Set(filteredItems.map(i => i.name));
    const storedNames = new Set(parsed.map(i => i.name));
    const missingInStorage = filteredItems.filter(i => !storedNames.has(i.name));
    const extraInStorage = parsed.filter(i => !iconNames.has(i.name));

    if (missingInStorage.length > 0 || extraInStorage.length > 0) {
      const merged = [
        ...parsed.filter(i => iconNames.has(i.name)),
        ...missingInStorage,
      ];
  ...
  const [ChatbotExpand, setChatbotExpand] = useState(
    {expand: false, show: false, hide: false, focusItem: true, x: 0, y: 0, zIndex: 1,});
  ...
  const [ProyectosExpand, setProyectosExpand] = useState(
    {expand: false, show: false, hide: false, focusItem: true, x: 0, y: 0, zIndex: 1,});
  ...
  const textError = t('run.error', { file: RunInputVal || regErrorPopUpVal })
...
function handleShowInfolderMobile(name, type) { //important handleshow for in folder
  ...
  const contextValue = {
    ...
    ChatbotExpand, setChatbotExpand,
    ProyectosExpand, setProyectosExpand,
...
function ObjectState() {
  return [
    ...
    { name: 'Chatbot',     setter: setChatbotExpand,    usestate: ChatbotExpand,    color: 'rgba(52, 70, 143, 0.85)', size: 'small' },
    { name: 'Proyectos',   setter: setProyectosExpand,  usestate: ProyectosExpand,  color: 'rgba(86, 114, 122, 0.85)', size: 'small' },
    { name: 'Terminal',    setter: setTerminalExpand,   usestate: TerminalExpand,   color: 'rgba(0, 0, 0, 0.85)',     size: 'small' },
