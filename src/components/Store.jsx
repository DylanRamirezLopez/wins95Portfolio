...
function Store() {
  ...
  const hiddenStoreItems = ['Patch']
  const itemsInStore = iconInfo
    .filter(item => item.category)
    .filter(item => !hiddenStoreItems.includes(item.name))
    .filter(item => {
      // Search filter
      ...
      >
        ...
        >
          <div className="folder_barname">
            ...
            <span style={{top: '1px'}}>{t('store.title')}</span>
          </div>
        ...
        <div className="file_edit_container">
          <p>{t('store.file')}<span style={{left: '-23px'}}>_</span></p>
          <p>{t('store.edit')}<span style={{left: '-24px'}}>_</span></p>
          <p>{t('store.view')}<span style={{left: '-32px'}}>_</span></p>
          <p>{t('store.help')}<span style={{left: '-30px'}}>_</span></p>
        </div>
            ...
            {itemBeingSelected && (
              <>
                ...
                <p className='section_3_des'>
                  {t('store.size', { size: itemBeingSelected.size })}
                </p>
                <button
                  onClick={() => {
                ...
                >
                  ...
                  : !installed && (itemIsInstalling === itemBeingSelected.name) ? t('store.installing')
                  : t('store.install')
                  }
                    <div 
                      key={resetAnimation}
                ...
                >
                  {disableUninstall && (itemBeingUninstall === itemBeingSelected.name) ? t('store.uninstalling') : t('store.uninstall')}
                </button>
              </>
            )}
          </div>
