      ...
      >
        ...
        >
          ...
          <div className={userName ? 'Username_input_div_active' : 'Username_input_div_disabled'}>
            <div className="container_username">
              ...
              <form onSubmit={(e) => { e.preventDefault() }}>
                ...
                <input type="text" maxLength={20} placeholder={t('msn.usernamePlaceholder')}
                  value={userNameValue}
          ...
          <div className="file_edit_container-MSN">
            <p>{t('msn.file')}<span style={{ left: '-23px' }}>_</span></p>
            <p>{t('msn.edit')}<span style={{ left: '-24px' }}>_</span></p>
            <p>{t('msn.view')}<span style={{ left: '-32px' }}>_</span></p>
            <p>{t('msn.help')}<span style={{ left: '-30px' }}>_</span></p>
          </div>
          <div className='groove_div'>
            ...
            <span>{t('msn.displayName', { name: userNameValue || 'Anonymous' })}</span>
            <div className={`activate_bot ${chatBotActive ? 'active' : ''}`}
              onClick={() => setChatBotActive(!chatBotActive)}
            >
              <span>{chatBotActive? t('msn.botOnline') : t('msn.botOffline') }</span>
            </div>     
          </div>
          <div className="chat_to_div">
            <span>
              {t('msn.onlineUsers')} <span>{onlineUser}</span>
            </span>
          </div>
          ...
          <div className="enter_text_div">
            <textarea
              maxLength={100}
              placeholder={t('msn.messagePlaceholder')}
              value={chatValue}
          ...
          <div className="status_div">
            <p>
              {chatValue.trim().length > 0
                ? t('msn.typing', { name: userNameValue })
                : t('msn.lastMessage', { time: lastMessage })}
            </p>
