...
function TaskManager() {
  ...
  const bannedItems = ['Fortune', 'AiAgent', 'TaskManager', '3dObject', 'PixelPic', 'Nft', 'Note', 'IE']
      ...
      >
        <div className='folder_folder-taskmanager' 
          ...
          >
            ...
            >{t('taskManager.endTask')}</button>
            <button className='a'
                onClick={() => setShutdownWindow(true)
              }
            >{t('taskManager.shutDown')}</button>
            <button
              onClick={() => {
                setSelectedItemToClose('')
                setItemSelected(null);
              }}
            >{t('taskManager.cancel')}</button>
          </div>
        </div>
      </Draggable>
    </>
