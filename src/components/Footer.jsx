...
import { clippySpanish, clippySuggest, getRandomSpanish } from '../utils/ClippyFunction';
...
export default function Footer() {
    ...
    const [clippyCurrentPhrase, setClippyCurrentPhrase] = useState(getRandomSpanish('greeting'))
    const idleTimerRef = useRef(null)
    const prevTapLength = useRef(tap.length)

    function startIdleRotation() {
        clearInterval(idleTimerRef.current)
        idleTimerRef.current = setInterval(() => {
            const cats = ['idle', 'curious', 'idle', 'encouragement', 'idle']
            const cat = cats[Math.floor(Math.random() * cats.length)]
            const next = getRandomSpanish(cat)
            setClippyCurrentPhrase(next)
        }, 7000 + Math.random() * 5000)
    }

    useEffect(() => {
        if (showClippy) startIdleRotation()
        return () => clearInterval(idleTimerRef.current)
    }, [showClippy])

    useEffect(() => {
        if (tap.length > prevTapLength.current) {
            const next = getRandomSpanish('windowOpen')
            setClippyCurrentPhrase(next)
        }
        prevTapLength.current = tap.length
    }, [tap.length])
    ...
    function dontTouch() {
        ...
        const next = getRandomSpanish('click')
        setClippyCurrentPhrase(next)
        setTimeout(() => {
    ...
    function handleClipperTalk() {
        ...
        if (clippyTouched) return clippyCurrentPhrase.phrase
        return clippyCurrentPhrase.phrase
    }
        ...
        <>
            <div className="footer">
                    ...
                    <div className="icon_time_container">
                        ...
                        <div className="language_toggle"
                            ...
                            style={{
                                ...
                                fontWeight: 'bold',
                                padding: '2px 5px',
                                background: lang === 'es' ? '#040482' : 'transparent',
                                color: lang === 'es' ? 'white' : '#000',
                ...
                {showClippy && (
                    ...
                    >
                    <img src={clippyCurrentPhrase.animation} alt="clippy" />
